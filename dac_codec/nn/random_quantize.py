...
class RandomVectorQuantize(nn.Module):
    ...
    def __init__(self, input_dim: int, randn_codebook_size: int, embedding: nn.Embedding, codebook_dim: int):
        super().__init__()
...
class RandomResidualVectorQuantize(nn.Module):
    ...
    def __init__(
        ...
        self.big_embedding = nn.Embedding(self.codebook_size*4, self.codebook_dim[-1]).requires_grad_(False)

        self.quantizers = nn.ModuleList(
            [
                VectorQuantize(input_dim, codebook_size, codebook_dim[i])
                for i in range(n_codebooks - 4)
            ]
            +
            [
                RandomVectorQuantize(input_dim, codebook_size, self.big_embedding, codebook_dim[i])
                for i in range(n_codebooks-4, n_codebooks)
            ]
        )
